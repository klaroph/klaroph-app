-- Single RPC for founder dashboard: returns all founder metrics in one round-trip.
-- SECURITY DEFINER so it can read auth.users; call with service_role or from backend only.
-- Matches FOUNDER_DASHBOARD_ANALYTICS_SQL.md metrics 1–13.
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'active_budget_users', (SELECT COUNT(DISTINCT user_id)::int FROM public.budget_plans WHERE GREATEST(created_at, updated_at) >= (now() AT TIME ZONE 'UTC') - interval '90 days'),
