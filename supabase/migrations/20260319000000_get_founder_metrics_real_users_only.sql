-- Founder metrics: count only real users (profiles.user_type = 'user').
-- Adds real_users CTE and filters all user-based metrics. JSON shape unchanged.
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'auth_audit', (SELECT jsonb_build_object(
      ...
      'profiles_count', (SELECT COUNT(*)::int FROM real_users),
      'auth_minus_profiles', (SELECT COUNT(*)::int FROM auth.users) - (SELECT COUNT(*)::int FROM real_users),
...
COMMENT ON FUNCTION public.get_founder_metrics() IS 'Founder dashboard: all metrics in one call. Counts only real users (profiles.user_type = user). Use service_role or backend only.';
